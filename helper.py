...
class Helper:
    def __init__(self, current_time, params, name):
        ...
        self.sgd =Sgd(use_memory=self.params['sgd_use_memory'])
    ...
    def accumulate_weight(self, weight_accumulator, epochs_submit_update_dict, state_keys,num_samples_dict):
        """
         ...
         """
        if self.params['aggregation_methods'] == config.AGGR_FOOLSGOLD or self.params['aggregation_methods'] == config.AGGR_GRAD_MEAN:
            updates = dict()
    ...
    def average_shrink_models(self, weight_accumulator, target_model, epoch_interval):
        ...
        for name, data in target_model.state_dict().items():
            ...
            if update_per_layer.dtype!=data.dtype:
                update_per_layer=update_per_layer.int()
            data.add_(update_per_layer)
        return True

    def average_grad(self,target_model,updates):
        client_grads = []
        alphas = []
        names = []
        ...
        optimizer.zero_grad()
        agg_grads= self.sgd.aggregate_gradients(client_grads,names)

        for i, (name, params) in enumerate(target_model.named_parameters()):

            if params.requires_grad:
                agg_grads[i-2] = agg_grads[i-2] * self.params["eta"]
                params.grad = agg_grads[i-2].to(config.device)
            else:
                continue
        optimizer.step()
    ...
    def weighted_average_oracle(points, weights):
        ...
        weighted_updates=0
        for item in points:
            weighted_updates+=  item
        weighted_updates=weighted_updates/len(points)
...
class FoolsGold(object):
    ...
    def aggregate_gradients(self, client_grads,names):#client_grads为各个参与者梯度的集合
        cur_time = time.time()
...
class Sgd(object):
    ...
    def aggregate_gradients(self, client_grads,names):#client_grads为各个参与者梯度的集合
        weighted_updates = client_grads[0]
        weighted_updates_arr = np.array(weighted_updates)
        for item in client_grads[1:]:
            item=np.array(item)

            weighted_updates_arr=item+weighted_updates_arr
        weighted_updates_arr=weighted_updates_arr/len(client_grads)
        weighted_updates=weighted_updates_arr.tolist()
        return weighted_updates
