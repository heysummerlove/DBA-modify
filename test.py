...
def Mytest(helper, epoch,
    ...
    main.logger.info('___Test {} poisoned: {}, epoch: {}: Average loss: {:.4f}, '
                     'Accuracy: {}/{} ({:.4f}%)'.format( model._modules["1"].name,is_poison, epoch,
                                                        total_l, correct, dataset_size,
                                                        acc))
    if visualize: # loss =total_l
        model._modules['1'].test_vis(vis=main.vis, epoch=epoch, acc=acc, loss=None,
...
def Mytest_poison(helper, epoch,
    ...
    main.logger.info('___Test {} poisoned: {}, epoch: {}: Average loss: {:.4f}, '
                     'Accuracy: {}/{} ({:.4f}%)'.format(model._modules["1"].name, is_poison, epoch,
                                                        ...
                                                        acc))
    if visualize: #loss = total_l
        model._modules['1'].poison_test_vis(vis=main.vis, epoch=epoch, acc=acc, loss=None, eid=helper.params['environment_name'],agent_name_key=str(agent_name_key))
